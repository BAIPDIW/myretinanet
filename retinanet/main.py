...
import losses
from dataloader import XDataset,CocoDataset, CSVDataset, collater, Resizer, AspectRatioBasedSampler, Augmenter, UnNormalizer, Normalizer
...
def main(args=None):
    ...
    parser.add_argument('--dataset', default="x",help='Dataset type, must be one of csv or coco.')
    parser.add_argument('--x_path',default="./data/",help='Path to file contain training annotations')
    parser.add_argument('--coco_path', help='Path to COCO directory')
    parser.add_argument('--csv_train', default = "./data_aug/train_only.csv",help='Path to file containing training annotations (see readme)')
    parser.add_argument('--csv_classes', default = "./data_aug/classes.csv",help='Path to file containing class list (see readme)')
    parser.add_argument('--csv_val', default = "./data_aug/train_only.csv",help='Path to file containing validation annotations (optional, see readme)')
    ...
    elif parser.dataset == 'x':
        dataset_train = XDataset(parser.x_path, transform=transforms.Compose([Normalizer(), Augmenter(), Resizer()]))
    else:
        raise ValueError('Dataset type not understood (must be csv or coco), exiting.')

    #sampler = AspectRatioBasedSampler(dataset_train, batch_size=parser.batch_size, drop_last=False)
    dataloader_train = DataLoader(dataset_train, num_workers=2, collate_fn=collater, batch_size=parser.batch_size,shuffle=True,pin_memory=True)
    ...
    for epoch_num in range(parser.epochs):
        ...
        for iter_num, data in enumerate(dataloader_train):
            try:
                ...
                continue
        if (epoch_num + 1) % 5 == 0:
            torch.save(retinanet,'./logs/'+str(epoch_num)+'.pth')
